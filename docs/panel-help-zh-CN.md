...
1. 打开一个 C++ 源文件（.cpp）。
2. 在面板的「输入」栏输入测试数据，或在「预期输出」栏输入期望结果。
...
- 支持**拖拽** .in/.txt 文件到输入栏，或点击「载入文件」按钮选择文件。两种方式行为一致，无需区分。
- 大文件（>1MB）：仅加载前 64KB 作为预览显示（只读，标注「大文件预览，运行时完整读取」），运行时使用完整内容，不卡顿。
- 小文件（≤1MB）：完整载入，可直接在输入框编辑。
...
- 评测机参数（数据来源：洛谷 chen_zhe 2025-06-01：[https://www.luogu.com.cn/article/f3bqyl4w](https://www.luogu.com.cn/article/f3bqyl4w) 或 [https://www.luogu.me/article/f3bqyl4w](https://www.luogu.me/article/f3bqyl4w)）：
  - 洛谷: Intel Xeon Platinum 8369HC (GB6 单核 ~1472)
  - CCF 2025 CSP-S: Intel Core Ultra 9 285K (GB6 单核 ~2150)
    注意：CCF 曾更新评测机，2018-2024 用 i7-8700K (GB6 ~1614)，2025 CSP-S 起更换为 Core Ultra 9 285K（数据来源：洛谷 chen_zhe 2025-02-25：[https://www.luogu.com.cn/article/csoig4zt](https://www.luogu.com.cn/article/csoig4zt) 或 [https://www.luogu.me/article/csoig4zt](https://www.luogu.me/article/csoig4zt)）。
- 推荐在设置中手动指定设备 GeekBench 6 分数以获得更准确换算（cppRunner.userDeviceGeekbenchScore）。
- 查询设备分数：[https://browser.geekbench.com](https://browser.geekbench.com)
...
- 编译并运行（编辑器）: Ctrl+Shift+R / Cmd+Shift+R
